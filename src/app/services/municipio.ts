...
export class Municipio {
  ...
  url: string = `${environment.apiUrl}municipios`;
  ...
  crear(municipio: any): Observable<any> {
    const url = `${this.url}`;
    return new Observable(observer => {
      axios.post(url, municipio, {
        withCredentials: true,
        headers: this.headers
      })
        .then(response => {
  ...
  //actualizar elemento
  actualizar(mun_id: number, municipio: any): Observable<any> {
    const url = `${this.url}/${mun_id}`;
    return new Observable(observer => {
      axios.put(url, municipio, {
        withCredentials: true,
        headers: this.headers
      })
        .then(response => {
  ...
  //eliminar elemento
  eliminar(mun_id: number, mun_nombre: string): Observable<any> {
    const url = `${this.url}/${mun_id}`;
    return new Observable(observer => {
      axios.delete(url, {
        withCredentials: true,
