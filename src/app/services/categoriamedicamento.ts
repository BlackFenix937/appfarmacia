...
export class Categoriamedicamento {
  ...
  url: string = `${environment.apiUrl}categoriamedicamentos`;
  ...
  //detalle de elemento
  detalle(catmed_id: string | number | null = '', extra: string = ''): Observable<any> {
    const url = `${this.url}/` + catmed_id + extra;
    return new Observable(observer => {
      axios.get(url, {
        withCredentials: true,
        headers: this.headers
  ...
  //actualizar elemento
  actualizar(catmed_id: number, categoria: any): Observable<any> {
    const url = `${this.url}/${catmed_id}`;
    return new Observable(observer => {
  ...
  //eliminar elemento
  eliminar(catmed_id: number, catmed_nombre: string): Observable<any> {
    const url = `${this.url}/${catmed_id}`;
    return new Observable(observer => {
      axios.delete(url, {
        withCredentials: true,
